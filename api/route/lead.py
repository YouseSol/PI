...
class Message(pydantic.BaseModel):
    id: str
    role: t.Literal["LEAD"] | t.Literal["CLIENT"] | t.Literal["AGENT"]
    content: str
    timestamp: int
...
def get_chat(id: int, pi_api_token: t.Annotated[pydantic.UUID4, fastapi.Header()]) -> MessagesResponse:
    ...
    messages_in_chat = list()

    for message in chat_history:
        was_sent_by_the_system = MessageSentController.exist(lead=lead, message_id=message["id"])

        role = "AGENT" if was_sent_by_the_system else ("LEAD" if message["sender_id"] == lead.linkedin_public_identifier else "CLIENT")
...
@router.post("/{lead_id}/chat/{message_id}/feedback")
...
class MessageFeedback(pydantic.BaseModel):
    content: str | None

@router.get("/{lead_id}/chat/{message_id}/feedback")
def get_message_feedback(lead_id: int, message_id: str, pi_api_token: t.Annotated[pydantic.UUID4, fastapi.Header()]) -> MessageFeedback:
    client = ClientController.get_by_api_token(api_token=pi_api_token)
    ...
    return MessageFeedback(content=message_sent.feedback)
